...
def test_list_stock_date_filter(swell):
    ...
    date_filtered = swell.stock.list({
...
def test_list_stock_with_return_limit(swell):
    ...
    limit = 1
    response = swell.stock.list({"limit": limit})
...
def test_list_stock_expansion(swell):
    ...
    response = swell.stock.list({"expand": ["parent"]})
...
def test_get_stock_by_product_id(swell, stock_keys):
    ...
    id_response = swell.stock.get(first_product_stock_id)
...
def test_get_product_stock_expanded(swell, stock_keys):
    ...
    id_response = swell.stock.get(first_product_id, { "expand": ['parent'] })
...
def test_create_product_stock(swell, stock_keys):
    ...
    response = swell.stock.create(new_product_stock)
...
def test_fails_with_no_id(swell):
    ...
    with pytest.raises(ValueError):
        swell.stock.create({"quantity": 10})
...
def test_fails_with_no_quantity(swell):
    ...
    with pytest.raises(ValueError):
        swell.stock.create({"parent_id": "123"})
...
#     update_response = swell.stock.update(updates)
...
#         swell.stock.update({ "quantity": 10 })
...
#     with pytest.raises(TypeError):
#         swell.stock.update({ "id": 123, "quantity": 10 })
...
#     delete_response = swell.stock.delete(first_product_id)
